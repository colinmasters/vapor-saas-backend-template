...
public func configure(_ app: Application) async throws {
    ...
    // Configure auth provider - either Firebase or default Vapor JWT
    if let _ = Environment.get("FIREBASE_PROJECT_ID") {
        app.authProvider = FirebaseAuthProvider()
        app.logger.notice("Using Firebase authentication")
    } else {
        // Default Vapor JWT auth will be used
        app.logger.notice("Using default Vapor JWT authentication")
    }
